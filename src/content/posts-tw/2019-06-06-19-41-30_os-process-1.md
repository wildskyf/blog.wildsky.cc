...
slug: "os-process-1"
excerpt: "因為篇幅有點長，所以這一章節我分成上下兩篇文來寫。本篇會包含這些： Process vs Program Th..."
...
Process 執行完成，或被 OS 終止後，就會進入 terminated 狀態並釋放資源。

把完整流程串起來看：process 從 New 進入 Ready，取得 CPU 後成為 Running；Running 可能因時間片用完回到 Ready，也可能因等待 I/O 進入 Waiting，等 I/O 完成後再回到 Ready；工作完成後則進入 Terminated。
