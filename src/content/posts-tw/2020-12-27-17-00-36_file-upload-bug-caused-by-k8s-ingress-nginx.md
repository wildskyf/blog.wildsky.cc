...
這個 bug 和 Nginx 有關，它會 default 對檔案做 buffering，相關設定值為 `proxy_request_buffering` 可以在 [ngx_http_proxy_module 的文件](https://nginx.org/en/docs/http/ngx_http_proxy_module.html#proxy_request_buffering) 裡看到這個預設值為 `on`。
...
- [Module ngx_http_proxy_module proxy_request_buffering section](https://nginx.org/en/docs/http/ngx_http_proxy_module.html#proxy_request_buffering)
