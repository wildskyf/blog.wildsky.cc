...
slug: "os-process-2"
excerpt: "本章節承接 Process（上），會繼續談到下列這些東西： process scheduling proces..."
...
大致流程是：新的 process 先進入 Job queue，經 long-term scheduler 挑選後進入 Ready queue；short-term scheduler 再從 Ready queue 選出下一個使用 CPU 的 process。若執行中需要等待 I/O，就會進入對應的 Device queue，I/O 完成後再回到 Ready queue。
